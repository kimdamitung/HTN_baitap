	...
	while(1){
		...
		for(i = 0, count_of_changed_key = 0; i < sizeof(buttons) / sizeof(buttons[0]); i++){
			if(buttons[i] != current_buttons[i]){
				...
				if(buttons[0] == '1'){
					printf("So lan xuat hien number trong file: %d", check_text_lower_in_file(pFile, "alphabet.txt"));
				}else if(buttons[1] == '1'){
					convert_lower_to_upper(pFile, "alphabet.txt");
				}else if(buttons[2] == '1'){
